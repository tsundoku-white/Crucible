...
namespace n_pipeline
{
  ...
  VkShaderModule createShaderModule(Context &context, const std::vector<char>& code) {
    ...
    vkCheck(vkCreateShaderModule(context.m_device, &createInfo, nullptr, &shaderModule),
  ...
  void createPipeline(Pipeline &pipeline, Descriptor &descriptor, Context &context)
  {
    ...
    multisampling.rasterizationSamples  = context.m_msaaSamples;
    ...
    vkCheck(vkCreatePipelineLayout(context.m_device, &pipelineLayoutInfo, nullptr, &pipeline.m_layout), 
    ...
    vkCheck(vkCreateGraphicsPipelines(context.m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline.m_pipeline), 
